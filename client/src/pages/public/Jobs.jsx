...
import {
  ...
  X, ChevronDown, ChevronUp, ArrowRight, Filter,
  LayoutGrid, List, Sparkles, TrendingUp, RefreshCw,
  Eye, ChevronLeft, ChevronRight, Calendar, AlertCircle
} from "lucide-react";
...
const STYLES = `
  ...
  * { font-family: 'Figtree', sans-serif; }
  ...
  .expired-card { filter: grayscale(0.7); opacity: 0.6; }
`;
...
const JobCard = ({ job, index, onApply, gridView }) => {
  ...
  const endDate   = formatDate(job.lastDateToApply);

  if (!gridView) {
    return (
      <div className={`bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-5 card-hover animate-slide-up flex items-center gap-5 ${expired ? "expired-card" : ""}`} style={{ animationDelay: `${(index % 8) * 0.04}s`, opacity: 0 }}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-3 mt-2 text-xs text-slate-400">
            ...
            {endDate && (
              <span className={`flex items-center gap-1 font-semibold ${expired ? "text-red-500 dark:text-red-400" : "text-amber-600 dark:text-amber-400"}`}>
                <Clock size={11} /> {expired ? `Closed ${endDate}` : `Apply by ${endDate}`}
              </span>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          <Link to={`/jobs/${job._id}`} className="px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-700 text-sm font-semibold text-slate-700 dark:text-slate-300 hover:border-violet-400 hover:text-violet-600 transition-all">Details</Link>
          {!expired && (
            <button onClick={() => onApply(job)} className="px-4 py-2 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 text-white text-sm font-semibold shadow-lg shadow-violet-500/20 hover:shadow-violet-500/40 hover:scale-105 active:scale-95 transition-all">Apply</button>
          )}
        </div>
      </div>
    );
  ...
  return (
    <div className={`bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 overflow-hidden flex flex-col card-hover animate-slide-up ${expired ? "expired-card" : ""}`} style={{ animationDelay: `${(index % 9) * 0.05}s`, opacity: 0 }}>
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {job.location && <span className="inline-flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-lg"><MapPin size={10} /> {job.location}</span>}
          {(job.experience || job.experienceRequired) && <span className="inline-flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-2.5 py-1 rounded-lg"><Clock size={10} /> {job.experience || job.experienceRequired}</span>}
        </div>

        {/* Salary */}
        {salary && <div className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-bold text-sm mb-2"><IndianRupee size={12} /> {salary}</div>}
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {startDate && (
            <span className="inline-flex items-center gap-1 text-xs bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 px-2.5 py-1 rounded-lg font-medium">
              <Calendar size={10} /> From {startDate}
            </span>
          )}
          {endDate && (
            <span className={`inline-flex items-center gap-1 text-xs px-2.5 py-1 rounded-lg font-semibold ${expired ? "bg-red-100 dark:bg-red-900/20 text-red-600 dark:text-red-400" : "bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400"}`}>
              <Clock size={10} /> {expired ? `Closed ${endDate}` : `Apply by ${endDate}`}
            </span>
          )}
          {expired && (
            <span className="inline-flex items-center gap-1 text-xs bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-400 px-2.5 py-1 rounded-lg font-semibold">
              <AlertCircle size={10} /> Applications Closed
            </span>
          )}
        </div>
        ...
        {skills.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-4">
            {skills.slice(0, 3).map(s => <span key={s} className="text-[11px] px-2 py-0.5 rounded-lg bg-violet-50 dark:bg-violet-900/20 text-violet-700 dark:text-violet-400 border border-violet-100 dark:border-violet-800 font-medium">{s}</span>)}
            {skills.length > 3 && <span className="text-[11px] px-2 py-0.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-400">+{skills.length - 3}</span>}
          </div>
        )}
        ...
        <div className="mt-auto pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2">
          <Link to={`/jobs/${job._id}`} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:border-violet-400 hover:text-violet-600 transition-all">
            <Eye size={13} /> View Details
          </Link>
          {expired ? (
            <div className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-400 text-xs font-semibold cursor-not-allowed">
              Closed
            </div>
          ) : (
            <button onClick={() => onApply(job)} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 text-white text-xs font-semibold shadow-lg shadow-violet-500/20 hover:shadow-violet-500/40 hover:scale-[1.02] active:scale-[0.98] transition-all">
...
const Jobs = () => {
  ...
  useEffect(() => { setPage(1); }, [appliedQ, typeFilter, locFilter, sortBy]);
  ...
  const filtered = useMemo(() => {
    ...
    return res;
  }, [jobs, appliedQ, typeFilter, locFilter, sortBy]);

  const totalPages = Math.ceil(filtered.length / JOBS_PER_PAGE);
  const paginated  = filtered.slice((page - 1) * JOBS_PER_PAGE, page * JOBS_PER_PAGE);

  const handleSearch = () => { setAppliedQ(query.trim()); };

  const clearFilters = () => { setQuery(""); setAppliedQ(""); setTypeFilter("all"); setLocFilter("all"); setSortBy("newest"); };
  const hasFilters   = appliedQ || typeFilter !== "all" || locFilter !== "all";
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-5">
      ...
      <FilterSection title="Job Type">
        <div className="space-y-1.5">
          {jobTypes.map(t => (
            <button key={t} onClick={() => setTypeFilter(t)} className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-sm font-medium transition-all capitalize ${typeFilter === t ? "bg-violet-600 text-white" : "text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800"}`}>
      ...
      <FilterSection title="Location">
        <div className="space-y-1.5">
          {locations.slice(0, 8).map(l => (
            <button key={l} onClick={() => setLocFilter(l)} className={`w-full flex items-center justify-between px-3 py-2 rounded-xl text-sm font-medium transition-all ${locFilter === l ? "bg-violet-600 text-white" : "text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800"}`}>
              <span className="truncate text-left">{l === "all" ? "All Locations" : l}</span>
              {l !== "all" && <span className={`text-[11px] px-1.5 py-0.5 rounded-md font-semibold ml-2 flex-shrink-0 ${locFilter === l ? "bg-white/20 text-white" : "bg-slate-100 dark:bg-slate-800 text-slate-400"}`}>{jobs.filter(j => j.location === l).length}</span>}
            </button>
          ))}
        </div>
      </FilterSection>
      <FilterSection title="Sort By" defaultOpen={false}>
        <div className="space-y-1.5">
          {[{ v: "newest", l: "Newest First" }, { v: "oldest", l: "Oldest First" }].map(({ v, l }) => (
            <button key={v} onClick={() => setSortBy(v)} className={`w-full text-left px-3 py-2 rounded-xl text-sm font-medium transition-all ${sortBy === v ? "bg-violet-600 text-white" : "text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800"}`}>{l}</button>
          ))}
        </div>
      </FilterSection>
    </div>
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pt-20 pb-16 transition-colors duration-300">
      ...
      <div className="bg-gradient-to-br from-slate-900 via-violet-950 to-indigo-950 py-12 px-6 mb-8">
        <div className="max-w-4xl mx-auto text-center">
          ...
          {/* Search Bar — only fires on button click or Enter */}
          <div className="flex items-center bg-white dark:bg-slate-900 rounded-2xl p-2 shadow-2xl border border-white/10 max-w-2xl mx-auto">
            <div className="flex items-center gap-3 flex-1 px-3">
              ...
              />
              {query && <button onClick={() => { setQuery(""); setAppliedQ(""); }} className="text-slate-400 hover:text-slate-600 transition-colors"><X size={14} /></button>}
            </div>
            ...
            <div className="hidden sm:flex items-center gap-2 px-3 text-slate-400">
              <MapPin size={15} />
              <select value={locFilter} onChange={e => setLocFilter(e.target.value)} className="bg-transparent text-sm outline-none cursor-pointer dark:text-slate-300">
            ...
            <button
              ...
              className="ml-2 bg-gradient-to-r from-violet-600 to-indigo-600 text-white px-5 py-2.5 rounded-xl font-semibold text-sm hover:scale-105 active:scale-95 transition-all shadow-lg shadow-violet-500/30"
            >
              Search
            </button>
          </div>
          ...
          {jobTypes.filter(t => t !== "all").length > 0 && (
            <div className="flex flex-wrap justify-center gap-2 mt-4">
              {jobTypes.filter(t => t !== "all").slice(0, 5).map(t => (
                <button key={t} onClick={() => setTypeFilter(typeFilter === t ? "all" : t)} className={`px-4 py-1.5 rounded-full text-xs font-semibold capitalize transition-all border ${typeFilter === t ? "bg-violet-600 text-white border-violet-600" : "bg-white/10 text-slate-300 border-white/20 hover:bg-white/20"}`}>{t}</button>
              ))}
            </div>
          )}
        </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center justify-between mb-5 animate-slide-up">
          <div className="flex items-center gap-2">
            ...
            {hasFilters && <span className="text-xs px-2.5 py-1 bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-400 rounded-lg font-semibold border border-violet-200 dark:border-violet-800">Filtered</span>}
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => setMobileFilter(m => !m)} className="lg:hidden flex items-center gap-2 px-4 py-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 text-sm font-semibold hover:border-violet-400 transition-all">
              <Search size={15} /> Filters {hasFilters && <span className="w-2 h-2 rounded-full bg-violet-500" />}
            </button>
            <div className="flex bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-1">
              <button onClick={() => setGridView(true)}  className={`p-2 rounded-lg transition-all ${gridView  ? "bg-violet-600 text-white shadow-sm" : "text-slate-400 hover:text-slate-600"}`}><LayoutGrid size={15} /></button>
              <button onClick={() => setGridView(false)} className={`p-2 rounded-lg transition-all ${!gridView ? "bg-violet-600 text-white shadow-sm" : "text-slate-400 hover:text-slate-600"}`}><List size={15} /></button>
            </div>
          </div>
        </div>

        {mobileFilter && <div className="lg:hidden mb-5 animate-slide-up"><FilterPanel /></div>}
